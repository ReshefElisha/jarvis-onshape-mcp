...
# `sketchPlane` parameter, or in a constraint's external ref, means the
# sketch is bound to picked/generated geometry rather than a fixed default
# plane -- see `assess_sketch_risk` below.
STANDARD_PLANE_IDS = frozenset({"JCC", "JDC", "JEC"})
...
def assess_sketch_risk(
    features_doc: Dict[str, Any],
    feature_id: str,
) -> Optional[Dict[str, Any]]:
    """Assess whether a sketch has external geometry references that make it
    unsafe to re-POST via the GET-whole/patch-one-field/POST-whole-back
    mechanism `rename_feature`/`update_feature`/`edit_sketch` all use.

    Confirmed live, deterministically, three times: a sketch anchored to a
    face generated by a custom FeatureScript feature (a non-default plane
    reference, plus constraints referencing entities outside the sketch's
    own geometry) triggers SKETCH_SOLVE_FAILED and cascades into a full-tree
    collapse when touched this way. A sketch with zero external references
    round-trips safely. This function is the detector; callers decide what
    to do with a `risky=True` result.

    Returns None if `feature_id` isn't found in `features_doc`, or is found
    but isn't a BTMSketch-151 -- deliberately quiet (not a raise) since
    callers run this unconditionally on every rename/update target
    regardless of feature type.

    Otherwise always returns a populated dict (risky or not), so this stays
    independently useful/testable rather than only emitting on the risky
    path:
        {feature_id, name, is_sketch: True, risky,
         plane_ids, plane_is_default,
         risky_constraints: [{id, constraintType, externalFirst, externalSecond}, ...],
         summary}
    """
    feature: Optional[Dict[str, Any]] = None
    for f in features_doc.get("features") or []:
        if isinstance(f, dict) and f.get("featureId") == feature_id:
            feature = f
            break
    if feature is None or feature.get("btType") != "BTMSketch-151":
        return None
    ...
    plane_is_default = all(pid in STANDARD_PLANE_IDS for pid in plane_ids) if plane_ids else True

    risky_constraints: List[Dict[str, Any]] = []
    for c in feature.get("constraints") or []:
        summary = _summarize_constraint(c)
        if summary.get("externalFirst") or summary.get("externalSecond"):
            risky_constraints.append({
                "id": summary.get("id"),
                "constraintType": summary.get("constraintType"),
                "externalFirst": summary.get("externalFirst"),
                "externalSecond": summary.get("externalSecond"),
            })

    risky = (not plane_is_default) or bool(risky_constraints)

    if risky:
        parts = []
        if not plane_is_default:
            external_plane = [pid for pid in plane_ids if pid not in STANDARD_PLANE_IDS]
            parts.append(
                f"its sketch plane references non-default geometry (id(s) {external_plane}), "
                "not a fixed Front/Top/Right plane"
            )
        if risky_constraints:
            parts.append(
                f"{len(risky_constraints)} constraint(s) reference geometry outside the "
                f"sketch's own entities ({[c['constraintType'] for c in risky_constraints]})"
            )
        summary_text = (
            f"Sketch {feature.get('name') or feature_id!r} is anchored to external "
            f"geometry: {' and '.join(parts)}. This pattern has been confirmed live to "
            "trigger sketch solver failures cascading into roughly 20 downstream "
            "ERROR/WARNING features and a visibly collapsed model."
        )
    else:
        summary_text = (
            f"Sketch {feature.get('name') or feature_id!r} has no external geometry "
            "references (default plane, purely local constraints) -- safe to modify."
        )
    ...
    return {
        ...
        "risky": risky,
        "plane_ids": plane_ids,
        "plane_is_default": plane_is_default,
        "risky_constraints": risky_constraints,
