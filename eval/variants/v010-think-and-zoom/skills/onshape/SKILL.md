...
## Phase 2: build to the Phase 1 vision spec

The system prompt above includes a separate VISION SKILL covering Phase 1
(reference-image decomposition). Follow that skill first. After Phase 1
emits the structured FEATURE TREE, start Phase 2 here:

1. `create_document` + `create_part_studio`.
2. Build features in dependency order (base plate → subtractive cuts →
   bosses → fillets). The Phase 1 FEATURE TREE is the authoritative spec
   — if Phase 2 needs info that's not in the tree, you mis-scoped Phase 1.
3. After every 3-4 features, call
   `mcp__onshape__compare_to_reference(referenceImagePath=<brief's ref>, …)`
   to verify silhouette match. Zoom into any disagreement with `crop_image`
   and fix via `update_feature` or `delete_feature_by_name` + re-add.
4. Before export, do a final compare. Then `export_part_studio(format=STEP)`
   and reply DONE.
