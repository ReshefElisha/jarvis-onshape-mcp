...
## Compare to the reference image — side-by-side

When the brief ships with a reference PNG, call the **`compare_to_reference`**
tool instead of eyeballing your build against separate renders. Pass the
brief's `referenceImagePath` (it's the local filesystem path attached to your
prompt) plus your current Part Studio IDs, and the tool returns a single
composite PNG: reference on top, your build's iso/top/front/right row below
at the same horizontal extent. Both images in one ImageContent block — you
see silhouette mismatches, missing features, and wrong proportions at a
glance without toggling between images.

Call it:
- **After each major feature** that changes the silhouette (extrude, cut,
  boss, shell). Cheap: one tool call, ~2s latency.
- **Always before `export_part_studio`.** Don't ship a build you haven't
  compared end-to-end.

When the composite shows a mismatch, act on it: `crop_image` into the
disagreement region, diagnose (missing feature? wrong size? wrong placement?),
fix with `update_feature` or `delete_feature_by_name` + re-add. Don't keep
building on top of a silhouette that already diverges — the error
compounds feature by feature.
