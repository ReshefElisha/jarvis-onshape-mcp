...
## Compare to the reference at three checkpoints

When the brief ships with a reference image, call
`mcp__onshape__compare_to_reference` (passing the brief's
`referenceImagePath` and your part-studio IDs) at exactly these three
moments:

1. **After your base extrude.** Confirms your starting silhouette + size
   matches the reference at the gross level. Cheap; if this fails, fix
   before adding any more features.
2. **After roughly half your planned features.** Catches mid-build drift
   before it compounds.
3. **Right before `export_part_studio`.** Final sanity check.

The composite shows reference on top, your build's iso/top/front/right
below, both at the same horizontal extent. The agent-row label includes
your bbox in mm — cross-check against the reference's stated dimensions
(if any) and the overall envelope you see.

Don't compare more often than necessary; each call adds an ImageContent
to your context. Three checkpoints is the sweet spot.
