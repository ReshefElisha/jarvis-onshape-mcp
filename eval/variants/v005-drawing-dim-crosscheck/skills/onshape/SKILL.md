...
Before you touch Onshape:

1. **List every dimension callout** you can see in the drawing: overall
   length/width/height, hole diameters, hole positions from datums, pocket
   depths, fillet radii, etc. Write them out by label and magnitude.
2. **Identify the scale**. Drawings have a title-block scale (1:1, 2:1,
   1:2, etc.) and dimensions are the TRUE size, not scaled-from-paper.
   Trust the numeric callouts over any measurement you try to take off
   the pixel image.
3. **Cross-check the overall envelope.** The largest dimension on any axis
   is the overall size of the part along that axis. If you're about to
   extrude a 30 mm base but the drawing says 800 mm, stop and recount.
