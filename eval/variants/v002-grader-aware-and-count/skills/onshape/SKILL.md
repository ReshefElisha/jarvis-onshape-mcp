...
## How you are scored — aim higher than "looks right"

Your output is compared against a reference STEP. Six layers stack into one
composite score in [0, 1]:

- **L0 body exists** — gate, 0 if no solid.
- **L1 volume** — within ±5% for full credit. Degrades past ±20%.
- **L2 bounding box** — within ±5% per axis for full credit.
- **L3 topology signature** — ratio of face/edge/vertex counts vs reference.
  Full credit when ratio ≥ 0.9. Missing features tank this layer.
- **L4 Boolean IoU** — `vol(A ∩ B) / vol(A ∪ B)` after bbox-center alignment.
  Needs ≥ 0.95 for full credit; under 0.3 contributes nothing. This is
  where missing *or* wrong-placed features show up most.
- **L5 Chamfer distance** — mean surface-point distance normalized by bbox
  diagonal. Rewards geometric fidelity of silhouettes + curves.

**The failure mode to avoid: under-building.** A dimensionally-perfect
approximation with 3 features when the reference has 8 scores *worse* than
a rougher build with all 8 features at ±10%. Missing features cost 15–50%
of total score (L3 + L4 + L5 all drop at once). When in doubt, include the
feature. **Over-build rather than under-build.**

Corollary: the envelope numbers in the brief are *not* the target. Matching
the envelope satisfies L2 but is only 15% of the score. The other 85% lives
in the feature topology, which you must read from the render.
...
lives in the picture. Before you touch Onshape, write — in your own reply,
not in an Onshape doc — a short build plan:

1. **Decompose and count.** Before anything else, write one sentence:
   *"I count N features: 1 base plate, 3 through-holes, 2 bosses, 1 pocket,
   4 corner fillets."* If N feels small for what the render shows (≤ 4
   for a non-trivial part), zoom in with `crop_image` on the busy regions
   and recount. A typical medium-complexity part is 6–12 distinct features.
   This count is a commitment: if your post-build `describe_part_studio`
   reports fewer features than N, you are not done — keep building.
