...
## Plan from the reference FIRST — don't start building

If the brief ships with a reference image (isometric render, drawing, photo), the
image is ground truth. The text often only pins envelope + a hint; the geometry
lives in the picture. Before you touch Onshape, write out — in your own reply,
not in an Onshape doc — a short build plan:

1. **List the major features** you see in the reference. Count bosses, holes,
   cutouts, shells, fillets. Small count of distinct features is usually
   correct; if you list 3 features for a complex part, look again.
2. **Estimate proportions**. Relative to the stated envelope, what fraction of
   volume is each feature? Which face is each feature on?
3. **Pick a base**. The largest single extrusion (usually a plate or block) is
   feature #1. Everything else adds onto, cuts from, or shells that base.
4. **Order the feature queue**. Subtractive cuts that depend on a face's
   existence must come after that face is built.
5. **Only then** create the document and start building.

You will do worse if you skip step 1 and jump to `create_document`. The cost of
a 30-second visual plan is repaid many times over in fewer `update_feature`
...
## Drawings: read every callout BEFORE building

If the brief includes an engineering drawing (orthographic views with
numeric dimensions, GD&T frames, tolerances), your output is scored against
a STEP whose dimensions come directly from those callouts. **Misreading
scale by even 2× tanks every score layer.** A 4× under-read (thinking
200 mm when the drawing says 800 mm) produces L1 = L2 = L4 = L5 all zero.

**The drawing PNG attached inline to your prompt has been downsampled for
transport.** Dimension callouts often render at 6-10px tall — hard to
read reliably. DO THIS FIRST, before any build:

1. **Load the drawing into the image cache.** Call
   `mcp__onshape__load_local_image(imagePath=<the REFERENCE IMAGE path in
   your prompt>)`. You get back a stable `image_id` and the image comes
   back at native resolution.
2. **Crop into each dimension callout.** Use `crop_image(imageId=<id>,
   x1,y1,x2,y2)` to zoom into the regions of the drawing where numeric
   callouts live. Read the number AT NATIVE RESOLUTION. Do this for every
   labeled dimension: overall length/width/height, hole diameters, hole
   positions, pocket depths, fillet radii. Cropping is cheap — one call
   returns one number you're confident in.
3. **Write the callout table in your reply.** Before you `create_document`,
   enumerate the dimensions you read: `OVERALL: 800 x 450 x 150 mm`,
   `HOLES: Ø35 (4x at corners)`, `PADS: Ø25 (top center)`, etc. This
   commits you to those numbers and makes mis-reads obvious.
4. **Identify the scale**. Drawings have a title-block scale (1:1, 2:1,
   etc.) and dimensions are the TRUE size. Trust the numeric callouts
   over any size you'd estimate from the pixel image.

After every feature:

- Call `mcp__onshape__compare_to_reference` (or `get_bounding_box`) and
  read the bbox stamp on the composite. It's labeled in mm. Cross-check
  against the callout list.
- If bbox is off by more than 10% on any axis, STOP. Either you misread
  a dimension or you built at the wrong scale. Fix before adding more
  features — an undersized base propagates undersize to every feature
  on top of it.
