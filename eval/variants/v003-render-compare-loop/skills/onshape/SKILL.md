...
## Compare to the reference render every 3-4 features

Planning is not enough. Most of your scoring loss comes from feature
*placement* and *count* errors that can't be detected from text alone — the
brief shows you a render but you don't keep checking against it as you
build.

After every 3-4 features (or any feature that significantly changes the
silhouette), do a 3-step compare:

1. **Render your in-progress part**: call `describe_part_studio` with views
   `["iso", "front", "top", "right"]`. The response embeds 4-view PNGs of
   the current state.
2. **Open the reference image** that was attached to the brief, side-by-side
   with your render in your mental model. Compare:
   - Feature COUNT: are there features in the reference that aren't in your
     render? Bosses, holes, pockets, cuts you missed?
   - Feature PLACEMENT: are the features that exist in BOTH placed at the
     same fraction of the envelope? Same face? Same orientation?
   - Feature SIZE: is anything visually too big or too small relative to
     the overall part?
3. **Iterate before continuing.** If you see a missing feature, add it now
   — don't keep building on a wrong base. If a feature is misplaced, use
   `delete_feature_by_name` + re-add (`update_feature` only tweaks params,
   not sketch placement).

Two anti-patterns to avoid:

- **Open-loop building**: 60 turns of feature-after-feature, then one
  describe at the end. By then it's too late to fix the third feature
  cheaply.
- **Bbox-matching declarations**: "envelope is 305×98×250 ✓" sounds like
  success but is only ~15% of the score. The other 85% lives in feature
  topology + placement, which only the side-by-side image compare reveals.

When the part visually matches the reference at all 4 standard views,
THEN export.
